...
@Component({
  selector: 'jhi-export-cap',
  templateUrl: './export-cap.component.html',
  styleUrls: ['./export-cap.component.scss'],
})
export class ExportCapComponent extends UpdatePage implements OnInit {
  ...
  createExportObj(): any {
    const exportObj = {
      ...
      flgCapEnt: this.formExport.get('flgCapEnt')?.value ? this.formExport.get('flgCapEnt').value : null,
    };
    return exportObj;
  }
  ...
  sendExport(): void {
    if (this.formExport.valid) {
      ...
      this.exportComponent.exportEntity = 'CAP';
