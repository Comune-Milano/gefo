...
@Component({
  ...
  templateUrl: './diz-macro-prog.component.html',
  styleUrls: ['./diz-macro-prog.component.scss'],
})
export class DizMacroProgComponent extends SearchPage implements OnInit {
  /* @ts-ignore */
  @ViewChildren(NgbdSortableHeaderDirective) headers: QueryList<NgbdSortableHeaderDirective>;
  _exportObj!: IMacroProgetti;
  countFiltri: number;
  tipFinanziamenti$: Observable<ItipiFinanziamento[]> | undefined;
  macroProgetti!: Array<ProMacroProgResponse>;
  macroProgettiNoSort!: Array<ProMacroProgResponse>;
  highlightText = '';
  ...
  ngOnInit(): void {
    ...
    this.formFilter
      ...
      ?.valueChanges.pipe(
        ...
        switchMap((value: any) => this.macroProgService.ricercaMacroProgetto(this.formFilter.value))
      )
      .subscribe(
        (res: any) => {
  ...
  onSort({ column, direction }: SortEvent): void {
    ...
    // sorting
    if (direction === '' || column === '') {
      this.macroProgetti = JSON.parse(JSON.stringify(this.macroProgettiNoSort));
    } else {
      this.macroProgetti = this.macroProgetti.sort((a: any, b: any) => {
        let res = 0;
        switch (column) {
          case 'nroprofon':
          case 'imprisfon':
            ...
            break;
          case 'codtipfinDa':
            if (a.macroProgetto.proTipfinByIdTipfinda != null && b.macroProgetto.proTipfinByIdTipfinda != null) {
              res = compare(a.macroProgetto.proTipfinByIdTipfinda['codtipfin'], b.macroProgetto.proTipfinByIdTipfinda['codtipfin']);
            } else {
              res = a.macroProgetto.proTipfinByIdTipfinda == null ? 1 : -1;
            }
            break;
          case 'codtipfinA':
            ...
            break;
          default:
            res = compare(a.macroProgetto[column], b.macroProgetto[column]);
            break;
        }
  ...
  nuovoDizMacroProg(): void {
    this.router.navigateByUrl('/diz-macro-prog/new');
  }

  apriDettaglio(id: number): void {
    this.router.navigate([`diz-macro-prog/${id}/viewDetail`]);
  }

  exportToCSV(): void {
    this.exportTableCsvService.exportTableToCsv(
      this.translateService.instant('dizMacroProg.csvFileName'),
      this.convertDataCSV(this._exportObj)
    );
  }

  private convertDataCSV(queryResult?: IMacroProgetti): any[] {
    const items: any[] = [];

    queryResult?.macroProgetti.forEach(element => {
      const csvLine: any = {
        [this.translateService.instant('dizMacroProg.codmacpro')]: element.macroProgetto.codmacpro ? element.macroProgetto.codmacpro : '',
        [this.translateService.instant('dizMacroProg.desmacpro')]: element.macroProgetto.desmacpro ? element.macroProgetto.desmacpro : '',
        [this.translateService.instant('dizMacroProg.id_tipfinda')]: element.macroProgetto.proTipfinByIdTipfinda?.codtipfin
          ? element.macroProgetto.proTipfinByIdTipfinda.codtipfin
          : '',
        [this.translateService.instant('dizMacroProg.id_tipfina')]: element.macroProgetto.proTipfinByIdTipfina?.codtipfin
          ? element.macroProgetto.proTipfinByIdTipfina.codtipfin
          : '',
