...
@Component({
  ...
  templateUrl: './export-ava-fas.component.html',
  styleUrls: ['./export-ava-fas.component.scss'],
})
export class ExportAvaFasComponent extends UpdatePage implements OnInit {
  ...
  createExportObj(): any {
    const exportObj = {
      ...
      flgMil: this.formExport.get('flgMil')?.value ? Number(this.formExport.get('flgMil')?.value) : null,
    };
    return exportObj;
  }
  ...
  sendExport(): void {
    if (this.formExport.valid) {
      ...
      this.exportComponent.exportEntity = 'AVA';
