...
export class ExportDdrService {
  ...
  exportDdr(param: any): Observable<any> {
    return this.http
      .post(`services/dmifonbe/api/exportDdr/exportDdr?idUteRuo=${this.accountService.getCurIdUteRuo()}`, param, {
        headers: { skip: 'true' },
      })
      .pipe(
        catchError(err => {
